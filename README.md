...
## Хакатон: совместный досуг

Delim подходит для сценария «организатор поездки, похода или мероприятия ведёт
общий бюджет в MAX». Группа объединяет участников, расходы и чеки, а Core
рассчитывает баланс и план погашения. Материалы сдачи: [инструкция](docs/hackathon/submission.md),
[PDF-презентация](docs/hackathon/presentation.pdf), [архив исходников](docs/hackathon/delim-source.tar.gz)
и [контрольная сумма](docs/hackathon/delim-source.sha256).

Для воспроизводимого запуска после заполнения `.env` используйте `make dev-up`:
команда через Docker поднимает БД/MinIO, применяет миграции и запускает stack.
