...
from delim_document.receipt_lookup import ProverkaChekaClient
...
class App:
    ...
    async def run(self, stop_event: asyncio.Event) -> None:
        ...
        try:
            ...
            receipt_lookup = None
            if self._config.receipt_lookup.enabled:
                receipt_lookup = ProverkaChekaClient(
                    self._config.receipt_lookup.token or "",
                    endpoint=self._config.receipt_lookup.endpoint,
                    timeout_seconds=self._config.receipt_lookup.timeout_seconds,
                    max_response_bytes=self._config.receipt_lookup.max_response_bytes,
                    max_attempts=self._config.receipt_lookup.max_attempts,
                    retry_delay_code_2_seconds=(
                        self._config.receipt_lookup.retry_delay_code_2_seconds
                    ),
                    retry_delay_code_4_seconds=(
                        self._config.receipt_lookup.retry_delay_code_4_seconds
                    ),
                )
            worker = OCRWorker(
                jobs,
