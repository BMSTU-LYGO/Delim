...
@dataclass(frozen=True, slots=True)
class ReceiptLookupConfig:
    endpoint: str
    timeout_seconds: float
    max_response_bytes: int
    max_attempts: int
    retry_delay_code_2_seconds: float
    retry_delay_code_4_seconds: float
    token: str | None
...
def _positive_number(
    section: dict[str, Any], key: str, path: str, default: float
) -> float:
...
def _positive_int(section: dict[str, Any], key: str, path: str, default: int) -> int:
    value = section.get(key, default)
    if isinstance(value, bool) or not isinstance(value, int) or value <= 0:
        raise ConfigError(f"missing or invalid configuration value: {path}")
    return value
...
def load_config(path: str | Path) -> Config:
    ...
    receipt_lookup_value = loaded.get("receipt_lookup", {})
    if not isinstance(receipt_lookup_value, dict):
        raise ConfigError("missing or invalid configuration section: receipt_lookup")
    receipt_lookup = receipt_lookup_value
    receipt_lookup_endpoint = receipt_lookup.get(
        "endpoint", "https://proverkacheka.com/api/v1/check/get"
    )
    if (
        not isinstance(receipt_lookup_endpoint, str)
        or not receipt_lookup_endpoint.strip()
    ):
        raise ConfigError(
            "missing or invalid configuration value: receipt_lookup.endpoint"
        )
    ...
    return Config(
        ...
        receipt_lookup=ReceiptLookupConfig(
            ...
            timeout_seconds=_positive_number(
                receipt_lookup,
                "timeout_seconds",
                "receipt_lookup.timeout_seconds",
                10,
            ),
            max_response_bytes=_positive_int(
                receipt_lookup,
                "max_response_bytes",
                "receipt_lookup.max_response_bytes",
                1_048_576,
            ),
            max_attempts=_positive_int(
                receipt_lookup,
                "max_attempts",
                "receipt_lookup.max_attempts",
                5,
            ),
            retry_delay_code_2_seconds=_positive_number(
                receipt_lookup,
                "retry_delay_code_2_seconds",
                "receipt_lookup.retry_delay_code_2_seconds",
                2,
            ),
            retry_delay_code_4_seconds=_positive_number(
                receipt_lookup,
                "retry_delay_code_4_seconds",
                "receipt_lookup.retry_delay_code_4_seconds",
                8,
            ),
            token=os.getenv("PROVERKACHECKA_TOKEN", "").strip() or None,
        ),
        metrics=MetricsConfig(
            host=metrics_host,
            port=metrics_port,
